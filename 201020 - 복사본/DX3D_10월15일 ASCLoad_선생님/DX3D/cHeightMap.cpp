...
void cHeightMap::Setup(char * szFolder, char * szRaw, char * szTex, DWORD dwBytesPerPixel)
{
	...
	int nRow = (int)(sqrt((float)nNumVertex) + 0.00001f);
	int nCol = nRow;
	int nTileN = nRow - 1;
	m_nTileN = nTileN;
	...
	for (int i = 0; i < nNumVertex; i++)
	{
		...
		v.p = D3DXVECTOR3(i % nCol,
			(unsigned char)fgetc(fp) / 10.f, i / nCol);
	...
	for (int x = 1; x < nTileN; x++)
	{
		for (int z = 1; z < nTileN; z++)
		{
			...
			D3DXVECTOR3 leftToRight = m_vecVertex[right]
				- m_vecVertex[left];
			D3DXVECTOR3 downToUp = m_vecVertex[up]
				- m_vecVertex[down];
